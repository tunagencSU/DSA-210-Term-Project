...
def hava_durumu_olustur(isim, hedef_rakim, istasyonlar, baslangic, bitis):
    ...
    # Verileri Birleştirme ve Haftalık Özet
    haftalik_index = pd.date_range(start=baslangic, end=bitis, freq='W')
    
    if not dfs:
        print(f"\nKritik Uyarı: {isim.upper()} için hiçbir veri işlenemedi. Tablo boş (NaN) dönecek.")
        weekly_data = pd.DataFrame(index=haftalik_index)
        for f in features:
            weekly_data[f'{isim}_{f}'] = np.nan
        weekly_data[f'{isim}_snow'] = np.nan
        weekly_data[f'{isim}_rain'] = np.nan
    else:
        ...
        # Haftalık Özet (Rüzgar ve Nem dahil edildi)
        weekly_data = merged_data.resample('W').agg({
            f'{isim}_temp': 'mean', 
            f'{isim}_prcp': 'sum',   
            f'{isim}_snow': 'sum',   
            f'{isim}_rain': 'sum',   
    ...
    # Dosya Kaydı
    dosya_adi = f"- {isim}_final_hava.csv"
    ...
    weekly_data.to_csv(dosya_adi)
    print(f"\n BAŞARILI! Veriler '{dosya_adi}' dosyasına kaydedildi!")
